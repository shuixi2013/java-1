...
public class ThreadUtilsTest {
    ...
    public void testNoThread() throws InterruptedException {
        Assert.assertEquals(0, ThreadUtils.findThreadsByName("some_thread_which_does_not_exist_18762ZucTT").size());
    }

    @Test
    public void testNoThreadGroup() throws InterruptedException {
        Assert.assertEquals(0, ThreadUtils.findThreadGroupsByName("some_thread_group_which_does_not_exist_18762ZucTTII").size());
    }
    ...
    public void testThreadsSameName() throws InterruptedException {
        ...
        try {
            t1.start();
            alsot1.start();
            Assert.assertEquals(2, ThreadUtils.findThreadsByName("thread1_XXOOLL__").size());
        } finally {
            t1.interrupt();
            alsot1.interrupt();
            t1.join();
    ...
    public void testThreads() throws InterruptedException {
        ...
        try {
            t1.start();
            t2.start();
            Assert.assertEquals(1, ThreadUtils.findThreadsByName("thread2_XXOOLL__").size());
        } finally {
            t1.interrupt();
            t2.interrupt();
            t1.join();
    ...
    public void testThreadsById() throws InterruptedException {
        ...
        try {
            t1.start();
            t2.start();
            Assert.assertSame(t1, ThreadUtils.findThreadById(t1.getId()));
            Assert.assertSame(t2, ThreadUtils.findThreadById(t2.getId()));
        } finally {
            t1.interrupt();
            t2.interrupt();
            t1.join();
    ...
    public void testThreadsByIdWrongGroup() throws InterruptedException {
        ...
        try {
            t1.start();
            Assert.assertNull(ThreadUtils.findThreadById(t1.getId(), tg));
        } finally {
            t1.interrupt();
            t1.join();
            tg.destroy();
    ...
    public void testThreadGroups() throws InterruptedException {
        ...
        try {
            t1.start();
            t2.start();
            Assert.assertEquals(1, ThreadUtils.findThreadsByName("thread1_XXOOPP__").size());
            Assert.assertEquals(1, ThreadUtils.findThreadsByName("thread1_XXOOPP__","thread_group_DDZZ99__").size());
            Assert.assertEquals(1, ThreadUtils.findThreadsByName("thread2_XXOOPP__","thread_group_DDZZ99__").size());
            Assert.assertEquals(0, ThreadUtils.findThreadsByName("thread1_XXOOPP__","non_existent_thread_group_JJHHZZ__").size());
            Assert.assertEquals(0, ThreadUtils.findThreadsByName("non_existent_thread_BBDDWW__","thread_group_DDZZ99__").size());
            Assert.assertEquals(1, ThreadUtils.findThreadGroupsByName("thread_group_DDZZ99__").size());
            Assert.assertEquals(0, ThreadUtils.findThreadGroupsByName("non_existent_thread_group_JJHHZZ__").size());
            Assert.assertNotNull(ThreadUtils.findThreadById(t1.getId(),threadGroup));
        } finally {
            t1.interrupt();
            t2.interrupt();
            t1.join();
    ...
    public void testThreadGroupsRef() throws InterruptedException {
        ...
        try {
            ...
            t2.start();
            Assert.assertEquals(1, ThreadUtils.findThreadsByName("thread1_XXOOPP__").size());
            Assert.assertEquals(1, ThreadUtils.findThreadsByName("thread1_XXOOPP__",threadGroup).size());
            Assert.assertEquals(1, ThreadUtils.findThreadsByName("thread2_XXOOPP__",threadGroup).size());
            Assert.assertEquals(0, ThreadUtils.findThreadsByName("thread1_XXOOPP__",deadThreadGroup).size());
        } finally {
            t1.interrupt();
            t2.interrupt();
            t1.join();
            t2.join();
            threadGroup.destroy();
            Assert.assertEquals(0, ThreadUtils.findThreadsByName("thread2_XXOOPP__",threadGroup).size());
        }
    }
    ...
    public void testThreadGroupsById() throws InterruptedException {
        ...
        try {
            t1.start();
            t2.start();
            Assert.assertSame(t1, ThreadUtils.findThreadById(t1.getId(),"thread_group_DDZZ99__"));
            Assert.assertSame(t2, ThreadUtils.findThreadById(t2.getId(),"thread_group_DDZZ99__"));
            Assert.assertNull(ThreadUtils.findThreadById(nonExistingId,"non_existent_thread_group_JJHHZZ__"));
            Assert.assertNull(ThreadUtils.findThreadById(nonExistingId,"thread_group_DDZZ99__"));
        } finally {
            t1.interrupt();
            t2.interrupt();
            t1.join();
    ...
    public void testComplexThreadGroups() throws Exception {
        ...
        try {
            ...
            Assert.assertEquals(1, ThreadUtils.findThreadsByName(t4.getName(), threadGroup3.getName()).size());
            Assert.assertEquals(0, ThreadUtils.findThreadsByName(t4.getName(), threadGroup2.getName()).size());
            Assert.assertEquals(2, ThreadUtils.findThreadsByName(t11.getName(), threadGroup7.getName()).size());
        }finally {
            for (final Thread thread : threads) {
                thread.interrupt();
                thread.join();
