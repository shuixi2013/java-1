...
public class NotImplementedExceptionTest {
    ...
    private void assertCorrect(final String assertMessage, final NotImplementedException nie, final String message, final Throwable nested, final String code) {
        Assert.assertNotNull(assertMessage + ": target is null", nie);
        Assert.assertEquals(assertMessage + ": Message not equal", message, nie.getMessage());
        Assert.assertEquals(assertMessage + ": Nested throwable not equal", nested, nie.getCause());
        Assert.assertEquals(assertMessage + ": Code not equal", code, nie.getCode());
    }
}
