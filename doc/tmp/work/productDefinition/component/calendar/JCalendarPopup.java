 ...
 * Copyright 漏 2012 jbundle.org. All rights reserved.
 */
...
public class JCalendarPopup extends JPanel implements MouseListener {
	...
	protected DateFormat dateFormat = DateFormat
	...
	public void init(String strDateParam, Date dateTarget, String strLanguage) {
		...
		if (dateTarget == nowDate)
			selectedDate = targetDate; // If they want a default date, make the
										// time 12:00
	}
	...
	public void layoutCalendar(Date dateTarget) {
		...
		Date dateCalendarFirstDate = this
				.getFirstDateInCalendar(dateFirstOfMonth);
		Date dateCalendar = new Date(dateCalendarFirstDate.getTime());

		targetComponent = (int) ((targetDate.getTime() - dateCalendarFirstDate
				.getTime()) / KMS_IN_A_DAY);
		if (targetComponent < 0)
			targetComponent--;

		String strYear = this.getDateString(dateTarget, DateFormat.YEAR_FIELD);
		String strMonth = this
				.getDateString(dateTarget, DateFormat.MONTH_FIELD);
		...
		for (; iDayComponent < panelDays.getComponentCount(); iDayOfWeekComponent++, iDayComponent++) {
			if (iDayOfWeekComponent < 7) {
				JLabel labelDayOfWeek = (JLabel) panelDays
						.getComponent(iDayOfWeekComponent);
				String strWeek = this.getDateString(dateCalendar,
						DateFormat.DAY_OF_WEEK_FIELD);
//				if ((strWeek != null) && (strWeek.length() > 0))
//					labelDayOfWeek.setText(strWeek.substring(0, 1));
//				else
//					labelDayOfWeek.setText(Integer
//							.toString(iDayOfWeekComponent));
				labelDayOfWeek.setText("日,一,二,三,四,五,六".split(",")[iDayOfWeekComponent]);
			}
			JLabel labelDay = (JLabel) panelDays.getComponent(iDayComponent);
			String strDay = this.getDateString(dateCalendar,
	...
	private void initComponents() {
		...
		try {
			Icon icon = new ImageIcon(
					cl.getResource("org/jbundle/util/jcalendarbutton/images/buttons/"
							+ "Back" + ".gif"));
	...
	public String getDateString(Date dateTarget, int iDateFormat) {
		...
		String string = null;
		string = dateFormat.format(dateTarget, stringBuffer, fieldPosition)
				.toString();
	 ...
	 *            The calling button.
	 */
	public static JCalendarPopup createCalendarPopup(Date dateTarget,
			Component button) {
		return JCalendarPopup.createCalendarPopup(null, dateTarget, button,
				null);
	}
	 ...
	 *            The calling button.
	 */
	public static JCalendarPopup createCalendarPopup(String strDateParam,
			Date dateTarget, Component button) {
		return JCalendarPopup.createCalendarPopup(null, dateTarget, button,
				null);
	}
	 ...
	 */
	public static JCalendarPopup createCalendarPopup(String strDateParam,
			Date dateTarget, Component button, String strLanguage) {
		JPopupMenu popup = new JPopupMenu();
		JComponent c = (JComponent) popup; // ?.getContentPane();
		c.setLayout(new BorderLayout());
		JCalendarPopup calendar = new JCalendarPopup(strDateParam, dateTarget,
				strLanguage);
	 ...
	 *            The initial date for this button.
	 */
	public static JButton createCalendarButton(String strDateParam,
			Date dateTarget) {
